...
# Load dress image
dress_path = r"C:\virtualtryon\blue-removebg-preview.png"  # Path to the PNG image with a transparent background
...
cv2.namedWindow("Virtual Try-On Comparison", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Virtual Try-On Comparison", 1600, 1000)  # Set the window size

# Define padding for the T-shirt
padding = 20  # Add some padding to the dimensions

# Function to draw landmarks including specific points
...
while True:
    ...
    if results.pose_landmarks:
        ...
        if width_of_shirt > 0 and height_of_shirt > 0:
            ...
            frame_with_landmarks = overlayPNG(frame_with_landmarks, transformed_dress, (0, 0))
            draw_landmarks(frame_with_landmarks, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)

            frame_without_landmarks = frame.copy()
            frame_without_landmarks = overlayPNG(frame_without_landmarks, transformed_dress, (0, 0))

            combined_frame = np.hstack((frame_with_landmarks, frame_without_landmarks))

            cv2.imshow("Virtual Try-On Comparison", combined_frame)
